...
class LisensiService
{
    ...
    public static function sisaHari(): int
    {
        $row = self::getLisensiRow();
        if (! $row || ! $row->installed_at) {
            return (int) config('lisensi.trial_hari', 30);
        }
    ...
    public static function getTrialEndDate(): ?Carbon
    {
        $row = Lisensi::first();
        if (! $row || ! $row->installed_at) {
            return null;
    ...
    public static function isReadOnlyMode(): bool
    {
        ...
        if (self::isYearLicensed((int) $tahun->tahun)) {
            return false;
        }
