...
<div class="p-5 md:p-6 max-w-[1440px] mx-auto">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-3 mb-5">
        <div>
            <h1 class="text-lg md:text-xl font-bold text-slate-800">Master Data &mdash; Program / Kegiatan (8 SNP)</h1>
            <p class="text-xs text-slate-500 mt-0.5">Kelola kode kegiatan ARKAS menurut Standar Nasional Pendidikan.</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div class="px-4 py-3 border-b border-slate-200">
                    <h3 class="text-sm font-bold text-slate-700">Tambah Program</h3>
                </div>
                <form method="POST" action="{{ route('master.program.store') }}" class="p-4 space-y-3">
                    @csrf
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 mb-1">Kode <span class="text-red-500">*</span></label>
                        <input type="text" name="kode" required placeholder="03.02.01" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    <div>
        ...
        <div class="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="px-4 py-3 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <h3 class="text-sm font-bold text-slate-700">{{ $items->count() }} Program Terdaftar</h3>
                <form method="GET" class="flex items-center gap-2">
                    <input type="text" name="q" value="{{ $q }}" placeholder="Cari kode / nama / SNP..." class="rounded-lg border border-slate-300 px-3 py-1.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            ...
            <div class="max-h-[540px] overflow-y-auto divide-y divide-slate-100">
                ...
                @empty
                    <div class="px-4 py-8 text-center text-xs text-slate-400">Belum ada program. Tambahkan atau import dari Excel.</div>
                @endforelse
            </div>
        </div>
    </div>
