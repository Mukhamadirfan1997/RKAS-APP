...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <!-- Pagu Anggaran — Detail untuk TA yang sedang dilihat -->
        <div class="card overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700/60">
                <h2 class="text-sm font-bold text-slate-700 dark:text-slate-200">Anggaran TA {{ $tahunAnggaran->tahun ?? 2026 }} — Atur Pagu</h2>
                <p class="text-[11px] text-slate-400 dark:text-slate-500">Edit pagu untuk TA yang sedang dilihat ({{ $tahunAnggaran->tahun }}, {{ $tahunAnggaran->is_active ? 'Aktif' : 'Arsip' }}). Ganti TA aktif via <b>Daftar Tahun</b> di bawah.</p>
            </div>
        ...
        <!-- Status RKAS (Pengesahan & Kunci) -->
        <div class="card overflow-hidden lg:col-span-2" x-data="{ showSahkan:false, showBuka:false }">
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700/60 flex items-center justify-between">
                <div>
                    <h2 class="text-sm font-bold text-slate-700 dark:text-slate-200">Status RKAS</h2>
                    <p class="text-[11px] text-slate-400 dark:text-slate-500">Pengesahan &amp; kunci data — cegah edit setelah disahkan.</p>
                </div>
                @php
                    $status = $tahunAnggaran->status_pengesahan ?? 'Draft';
                @endphp
        ...
        <!-- Kelola Tahun Anggaran — Arsip & Ganti Aktif -->
        <div class="card overflow-hidden lg:col-span-2">
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700/60 flex items-center justify-between">
                <div>
                    <h2 class="text-sm font-bold text-slate-700 dark:text-slate-200">Kelola Tahun Anggaran — Arsip & Ganti Aktif</h2>
                    <p class="text-[11px] text-slate-400 dark:text-slate-500">Buat TA baru, lihat arsip, atau ganti tahun aktif (1 saja, ala ARKAS tahun_aktif). Pagu di atas untuk TA terpilih.</p>
                </div>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50">
                            ...
                            <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-800/40 @if($taRow->tahun == $tahunAnggaran->tahun) bg-blue-50/40 dark:bg-blue-500/10 @endif">
                                ...
                                <td class="px-3 py-2 text-center flex items-center justify-center gap-1">
                                    <a href="{{ route('pengaturan.index', ['tahun'=>$taRow->tahun]) }}" class="px-2 py-1 rounded-lg text-xs font-semibold text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-500/10">Lihat</a>
