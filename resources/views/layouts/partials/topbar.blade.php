<header class="flex items-center justify-between gap-3 px-4 lg:px-6 h-16 bg-white dark:bg-slate-800/95 border-b border-slate-200 dark:border-slate-700/60 shrink-0">
    ...
    <div class="flex items-center gap-2">
        ...
        @auth
        <form method="POST" action="{{ route('logout') }}" class="inline" onsubmit="try{localStorage.setItem('karsa_break_last', String(Date.now())); localStorage.removeItem('karsa_break_snooze');}catch(e){}">
            @csrf
